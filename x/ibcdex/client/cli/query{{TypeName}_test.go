...
import (
	...
	"github.com/username/interchange/testutil/network"
	"github.com/username/interchange/x/ibcdex/client/cli"
...
func networkWithDenomTraceObjects(t *testing.T, n int) (*network.Network, []*types.DenomTrace) {
	...
	for i := 0; i < n; i++ {
		state.DenomTraceList = append(state.DenomTraceList, &types.DenomTrace{Creator: "ANY", Index: strconv.Itoa(i)})
	}
	...
	for _, tc := range []struct {
		...
		t.Run(tc.desc, func(t *testing.T) {
			...
			if tc.err != nil {
				...
				require.Equal(t, tc.obj, resp.DenomTrace)
			}
		})
	}
}
...
func TestListDenomTrace(t *testing.T) {
	...
	t.Run("ByOffset", func(t *testing.T) {
		...
		for i := 0; i < len(objs); i += step {
			...
			for j := i; j < len(objs) && j < i+step; j++ {
				assert.Equal(t, objs[j], resp.DenomTrace[j-i])
			}
		}
	})
	...
	t.Run("Total", func(t *testing.T) {
		...
		require.Equal(t, objs, resp.DenomTrace)
	})
}
